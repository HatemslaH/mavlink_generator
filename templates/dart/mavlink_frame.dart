...
class MavlinkFrame {
  ...
  Uint8List _serializeV2() {
    ...
    var payload = _trimTrailingZeros(message.serialize());
    var payloadLength = payload.lengthInBytes;
    var messageIdBytes = [
  ...
  Uint8List _trimTrailingZeros(ByteData payload) {
    final payloadBytes = payload.buffer.asUint8List(payload.offsetInBytes, payload.lengthInBytes);

    var trimmedLength = payloadBytes.length;
    while (trimmedLength > 0 && payloadBytes[trimmedLength - 1] == 0) {
      trimmedLength--;
    }

    if (trimmedLength == payloadBytes.length) {
      return payloadBytes;
    }

    return Uint8List.sublistView(payloadBytes, 0, trimmedLength);
  }
}

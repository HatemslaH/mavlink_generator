#ifndef MAVLINK_FRAME_H
#define MAVLINK_FRAME_H
...
#define MAVLINK_MAX_FRAME_SIZE (12 + 255 + 2)
...
static inline size_t mavlink_trim_trailing_zeros(const uint8_t *payload, size_t payload_len) {
...
static inline size_t mavlink_frame_serialize_v2(
  ...
  out[7] = (uint8_t)(frame->message_id & 0xff);
  out[8] = (uint8_t)((frame->message_id >> 8) & 0xff);
  out[9] = (uint8_t)((frame->message_id >> 16) & 0xff);
  ...
  out[frame_len - 2] = (uint8_t)(mavlink_crc_x25_value(&crc) & 0xff);
  out[frame_len - 1] = (uint8_t)((mavlink_crc_x25_value(&crc) >> 8) & 0xff);
  return frame_len;
}

static inline void mavlink_frame_init_v2(
